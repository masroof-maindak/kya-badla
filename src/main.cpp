...
int main(int argc, char *argv[]) {
	...
	// Video loading
	auto video_colour_expected =
		read_frames(args.input_dir, args.input_ext, args.scale);
	if (!video_colour_expected.has_value()) {
		std::println(stderr, "Error reading frames: {}",
					 video_colour_expected.error());
		return 1;
	}
	...
	if (!video_gray_expected.has_value()) {
		std::println(stderr, "Error converting to grayscale: {}",
					 video_gray_expected.error());
		return 1;
	}
	...
	auto save_mean_expected =
		save_image(mean, args.output_dir, "mean", args.output_ext);
	if (!save_mean_expected.has_value()) {
		std::println(stderr, "Error saving mean image: {}",
					 save_mean_expected.error());
		return 1;
	}
	std::println("Computed and saved mean.");

	// Variance
	auto variance_expected =
		compute_variance(video_gray, mean, args.frame_count);
	if (!variance_expected.has_value()) {
		std::println(stderr, "Error computing variance: {}",
					 variance_expected.error());
		return 1;
	}
	auto variance = variance_expected.value();
	auto save_variance_expected =
		save_image(variance, args.output_dir, "variance", args.output_ext);
	if (!save_variance_expected.has_value()) {
		std::println(stderr, "Error saving variance image: {}",
					 save_variance_expected.error());
		return 1;
	}
